...
class UpdateProfileRequest extends FormRequest
{
    ...
    public function rules()
    {
        ...
        if(Auth::user()->can('change_state', User::class)) {
            $sensitiveData['state'] = 'required|in:' . implode(',', array_keys(User::getEnumsTrans('state')));;
        }

        if(Auth::user()->can('assign_roles', User::class)) {
            $sensitiveData['roles']= 'required|array|in:' . implode(',', array_keys(Role::getUserSelectOptions(true)->toArray()));
        }
