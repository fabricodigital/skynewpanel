...
<div class="panel panel-default">
    <div class="panel-body">
        <div class="row">
            @include('partials.inputs._text', ['name' => 'role_name', 'label' => $role::getAttrsTrans('name').'*'])
        </div>
        <div class="row">
            <div class="col-md-12 form-group @if($errors->has('level')) has-error @endif">
                ...
                {{ html()->number('level', null)->class('form-control') }}
                @include('partials._field-error', ['field' => 'level'])
            </div>
        </div>
        <div class="row">
            @include('partials.inputs._select-multi', ['name' => 'sub_roles', 'label' => $role::getAttrsTrans('sub_roles'), 'options' => $subRoles, 'selected' => $selectedSubRoles, 'multiple' => true])
        </div>
    </div>
</div>
